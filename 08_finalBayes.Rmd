...
To get some intuition of our Bayesian model, we could check out a regression model to see what's going on.

```{r eval = FALSE}
summary(glm(full_LiberalArts$Y2018~ full_LiberalArts$UGDS_1617 + full_LiberalArts$LOCALE_collapse_lac1 + full_LiberalArts$SAT_AVG_1617  + full_LiberalArts$ADM_RATE_1617, family = "poisson"))
```

### Some additional intuition

We could also build bi-variate visualizations that help us understand the individual trends: 
...
For even more intuition, we start with more vague priors and slightly adjust the priors based on the summary table. It's important to note that the priors cannot be set too close to the means of the summary table in case of **overfitting**.
...
Compared to the last attempts, our final model gives reasonable predictions, which is a huge progress! However, the 95% credible interval it provides doesn't provide an accurate enough interval as desired; we will keep working on adjusting priors.

By creating different imaginery universities and colleges, we will be able to summarize the most important factors for universities and colleges to respectively increase their rankings. (We are still working on this!)
